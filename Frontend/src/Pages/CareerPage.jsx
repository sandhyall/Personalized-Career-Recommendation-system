...
const CareerPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchCareer = async () => {
      ...
      try {
        const res = await axios.get(`${API_URL}/career/${slug}`);
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 text-sm max-w-xs mx-auto">
            We couldn't load details for this career. It may not exist or the
                  ...
                  {(data.advantages || []).length === 0 ? (
                    ...
                    <ul className="space-y-3">
                      {data.advantages.map((item, i) => (
                        <li
                          key={i}
                          className="text-sm text-slate-600 bg-emerald-50 p-3 rounded-xl border border-emerald-100"
                        >
                          {item}
                        </li>
                      ))}
                  ...
                  {(data.challenges || []).length === 0 ? (
                    ...
                    <ul className="space-y-3">
                      {data.challenges.map((item, i) => (
                        <li
                          key={i}
                          className="text-sm text-slate-600 bg-rose-50 p-3 rounded-xl border border-rose-100"
                        >
                          {item}
                        </li>
                      ))}
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-6 md:p-8 shadow-sm">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {data.real_projects.map((project, i) => (
                    <div
                      key={i}
                      className="group flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-transparent hover:border-indigo-200 hover:bg-white transition-all"
                    >
          ...
          <div className="space-y-6">
              ...
              {(data.tools || []).length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {data.tools.map((tool, i) => (
                    <span
                      key={i}
                      className="text-xs font-semibold bg-white/10 hover:bg-white/20 px-3 py-2 rounded-lg transition-colors border border-white/5"
                    >
                      {tool}
                    </span>
                  ))}
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                {!data.video_url && !data.pdf_url && !data.roadmap_url && (
                  <p className="text-sm text-slate-400 italic">
